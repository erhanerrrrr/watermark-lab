...
export function ResultsPage() {
  ...
  const detection = catalog.formal.detection
  const robustness = catalog.formal.robustness_v2
  const source = catalog.formal.data_source === 'local_formal_results' ? '本地 formal-v1 CSV' : '版本化正式结果快照'

  return <>
    <PageHeader eyebrow="结果分析" title="实验结果与正式对比" description="交互实验来自持久化数据库；总体、检测和扩展鲁棒性结论均读取冻结结果。" action={<a className={`secondary-button ${experiments.length ? '' : 'disabled'}`} href={experiments.length ? experimentExportUrl() : undefined} aria-disabled={!experiments.length}><Download size={16} /> 导出交互实验 CSV</a>} />
    ...
    <section className="results-grid">
      <div className="panel insight-panel">
        ...
        <h2>恢复准确不等于低误报</h2>
        <p>在 140 张 calibration 负样本上冻结 1% 目标阈值后，WAM 与 AM-WAM 在 690 张 test 负样本上的 FPR 分别为 {(detection.models.wam.fpr * 100).toFixed(2)}% 和 {(detection.models.am_wam.fpr * 100).toFixed(2)}%，TPR 均为 100%。</p>
        <div className="insight-stat"><strong>{(detection.models.am_wam.intrinsic_fpr * 100).toFixed(2)}%</strong><span>AM-WAM 默认规则的 clean 负样本 FPR</span></div>
        <StatusBadge tone="amber">TrustMark 仅提供二值检测分数</StatusBadge>
      </div>
      <div className="panel insight-panel">
        <span className="panel-kicker">robustness-v2 · {robustness.records.toLocaleString()} 条</span>
        <h2>离散候选外几何验证</h2>
        <p>{robustness.images} 张固定图像、{robustness.attack_cases} 条新攻击下，AM-WAM 相对 WAM 的 Bit Accuracy 提升 {robustness.innovation.bit_accuracy_gain_pp.toFixed(2)} pp，完整恢复率提升 {robustness.innovation.complete_recovery_gain_pp.toFixed(2)} pp。</p>
        <div className="insight-stat"><strong>+{robustness.innovation.off_grid_geometry_recovery_gain_pp.toFixed(2)} pp</strong><span>非候选网格几何攻击完整恢复率</span></div>
        <StatusBadge tone="amber">额外解码约 {robustness.innovation.decode_overhead_ms.toFixed(0)} ms</StatusBadge>
      </div>
    </section>
