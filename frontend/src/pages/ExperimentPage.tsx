...
function defaultAttack(catalog: ApiCatalog) {
  return catalog.interactive_attacks.find((attack) => attack.id === 'jpeg')
    ?? catalog.interactive_attacks[0]
}
...
export function ExperimentPage() {
  ...
  useEffect(() => {
    if (!catalog || catalog.interactive_attacks.some((item) => item.id === selectedAttack)) return
    const preferred = defaultAttack(catalog)
    if (preferred) {
      setSelectedAttack(preferred.id)
      setAttackParameter(preferred.default)
    }
  }, [catalog, selectedAttack])
  ...
  const reset = () => {
    const preferred = defaultModel(catalog)
    const preferredAttack = defaultAttack(catalog)
    setSelectedModel(preferred?.id ?? '')
    setStrength(preferred?.default_strength ?? 2)
    setSelectedAttack(preferredAttack?.id ?? '')
    setAttackParameter(preferredAttack?.default ?? 0)
    setMessage('WATERMARK-LAB · 2026')
    setFile(null)
    // Clearing React state alone leaves the native input value intact. Reset it
    // so selecting the same file again still emits a change event.
